""" Script "display_results_v3.py" from 12.05.2020.
    ...
    The output of the analysis (S_mode, S_90_limit, DSNB_mode, CCatmo_p_mode, CCatmo_C12_mode, Reactor_mode,
    NCatmo_mode) is saved in txt-files for each DM mass.
...
for DM_mass in DarkMatter_mass:
    ...
    file_CCatmo_p = path_result + "/CCatmo_p_mode_DMmass{0}.txt".format(DM_mass)
    ...
    file_CCatmo_C12 = path_result + "/CCatmo_C12_mode_DMmass{0}.txt".format(DM_mass)
    ...
    CCatmo_p_mode = np.loadtxt(file_CCatmo_p)
    Reactor_mode = np.loadtxt(file_reactor)
    NCatmo_mode = np.loadtxt(file_NCatmo)
    CCatmo_C12_mode = np.loadtxt(file_CCatmo_C12)
    ...
    """ Atmospheric CC background events on C12: """
    # Expected number of CCatmo background events from simulation (float):
    CCatmo_C12_expected = info_result[32]
    # Mean of the observed number of atmo. CC background events (float):
    CCatmo_C12_mean = info_result[33]
    # Standard deviation of the observed number of atmo. CC background events (float):
    CCatmo_C12_std = info_result[34]
    # 16 % confidence level of the observed number of atmo. CC background events (float):
    CCatmo_C12_16 = info_result[35]
    # 84 % confidence level of the observed number of atmo. CC background events (float):
    CCatmo_C12_84 = info_result[36]
    # -1 sigma value of the observed number of atmo. CC background events (float):
    CCatmo_C12_minus_1sigma = CCatmo_C12_mean - CCatmo_C12_std
    # +1 sigma value of the observed number of atmo. CC background events (float):
    CCatmo_C12_plus_1sigma = CCatmo_C12_mean + CCatmo_C12_std
    # 2.5 % confidence level of the observed number of atmo. CC background events (float):
    CCatmo_C12_2_5 = np.percentile(CCatmo_C12_mode, 2.5)
    # 97.5 % confidence level of the observed number of atmo. CC background events (float):
    CCatmo_C12_97_5 = np.percentile(CCatmo_C12_mode, 97.5)
    # -2 sigma value of the observed number of atmo. CC background events (float):
    CCatmo_C12_minus_2sigma = CCatmo_C12_mean - 2 * CCatmo_C12_std
    # +2 sigma value of the observed number of atmo. CC background events (float):
    CCatmo_C12_plus_2sigma = CCatmo_C12_mean + 2 * CCatmo_C12_std
    # 0.15 % confidence level of the observed number of atmo. CC background events (float):
    CCatmo_C12_0_15 = np.percentile(CCatmo_C12_mode, 0.15)
    # 99.85 % confidence level of the observed number of atmo. CC background events (float):
    CCatmo_C12_99_85 = np.percentile(CCatmo_C12_mode, 99.85)
    # -3 sigma value of the observed number of atmo. CC background events (float):
    CCatmo_C12_minus_3sigma = CCatmo_C12_mean - 3 * CCatmo_C12_std
    # +3 sigma value of the observed number of atmo. CC background events (float):
    CCatmo_C12_plus_3sigma = CCatmo_C12_mean + 3 * CCatmo_C12_std
    ...
    # add 95 % interval to the figure:
    plt.axvline(DSNB_2_5, linestyle='-.', label='2.5% probability limit = {0:.4f}'.format(DSNB_2_5),
                color='b')
    plt.axvline(DSNB_97_5, linestyle='-.', label='97.5% probability limit = {0:.4f}'.format(DSNB_97_5),
                color='b')
    ...
    # add 68 % interval to the figure:
    plt.axvline(DSNB_16, linestyle=':', label='16% probability limit = {0:.4f}'.format(DSNB_16), color='b')
    plt.axvline(DSNB_84, linestyle=':', label='84% probability limit = {0:.4f}'.format(DSNB_84), color='b')
    ...
    """ Display CCatmo_p_mean in histogram: """
    h4 = plt.figure(4, figsize=(15, 8))
    Bins4 = 'auto'
    # Bins4 = np.arange(0, 3, 0.1)
    n_CCatmo_p, bins4, patches4 = plt.hist(CCatmo_p_mode, bins=Bins4, histtype='step', color='b',
                                           label='number of virt. experiments = {0:.0f}'
                                           .format(number_of_entries))
    plt.axvline(CCatmo_p_expected, linestyle='dashed', label='expected number of events from simulation = {0:.3f}'
                .format(CCatmo_p_expected), color='r')
    plt.axvline(CCatmo_p_mean, linestyle='dashed', label='mean of the distribution = {0:.4f}'.format(CCatmo_p_mean),
                color='b')
    # define interpolation function:
    f_CCatmo_p = interp1d(bins4[:-1], n_CCatmo_p, kind='previous')
    bins4_small = np.arange(bins4[0], bins4[-2], 0.01)
    n_CCatmo_small = f_CCatmo_p(bins4_small)

    # add 95 % interval to the figure:
    plt.axvline(CCatmo_p_2_5, linestyle='-.', label='2.5% probability limit = {0:.4f}'.format(CCatmo_p_2_5),
                color='b')
    plt.axvline(CCatmo_p_97_5, linestyle='-.', label='97.5% probability limit = {0:.4f}'.format(CCatmo_p_97_5),
                color='b')
    # define a boolean array, which is True between CCatmo_2_5 and CCatmo_97_5 (array of bool):
    int_CCatmo_95percent = (bins4_small >= CCatmo_p_2_5) * (bins4_small <= CCatmo_p_97_5)
    plt.fill_between(bins4_small, 0, n_CCatmo_small, where=int_CCatmo_95percent, facecolor='yellow', alpha=1,
                     step='post')

    # add 68 % interval t the figure:
    plt.axvline(CCatmo_p_16, linestyle=':', label='16% probability limit = {0:.4f}'.format(CCatmo_p_16), color='b')
    plt.axvline(CCatmo_p_84, linestyle=':', label='84% probability limit = {0:.4f}'.format(CCatmo_p_84), color='b')
    # define a boolean array, which is True between CCatmo_16 and CCatmo_84 (array of bool):
    int_CCatmo_68percent = (bins4_small >= CCatmo_p_16) * (bins4_small <= CCatmo_p_84)
    plt.fill_between(bins4_small, 0, n_CCatmo_small, where=int_CCatmo_68percent, facecolor='green', alpha=1,
                     step='post')

    plt.xticks(np.arange(15, np.max(CCatmo_p_mode)+2.5, 2.5))
    plt.xlabel("total number of observed atmo. CC background events")
    plt.ylabel("counts")
    plt.title("Distribution of the observed number of atmospheric CC background events on protons"
              " in virtual experiments "
              "\n(for DM mass of {2:.1f} MeV and in the energy window from {0:.1f} MeV to {1:.1f} MeV)"
              .format(lower_energy_bound, upper_energy_bound, DM_mass))
    plt.legend()
    plt.savefig(path_result + "/result_CCatmo_p.png")
    plt.close()
    ...
    # add 95 % interval to the figure:
    plt.axvline(Reactor_2_5, linestyle='-.', label='2.5% probability limit = {0:.4f}'.format(Reactor_2_5),
                color='b')
    plt.axvline(Reactor_97_5, linestyle='-.', label='97.5% probability limit = {0:.4f}'.format(Reactor_97_5),
                color='b')
    ...
    plt.fill_between(bins5_small, 0, n_Reactor_small, where=int_Reactor_95percent, facecolor='yellow', alpha=1,
                     step='post')

    # add 68 % interval to the figure:
    plt.axvline(Reactor_16, linestyle=':', label='16% probability limit = {0:.4f}'.format(Reactor_16),
                color='b')
    plt.axvline(Reactor_84, linestyle=':', label='84% probability limit = {0:.4f}'.format(Reactor_84),
                color='b')
    ...
    plt.fill_between(bins5_small, 0, n_Reactor_small, where=int_Reactor_68percent, facecolor='green', alpha=1,
                     step='post')
    ...
    # add 95 % interval to the figure:
    plt.axvline(NCatmo_2_5, linestyle='-.', label='2.5% probability limit = {0:.4f}'.format(NCatmo_2_5),
                color='b')
    plt.axvline(NCatmo_97_5, linestyle='-.', label='97.5% probability limit = {0:.4f}'.format(NCatmo_97_5),
                color='b')
    ...
    plt.fill_between(bins8_small, 0, n_NCatmo_small, where=int_NCatmo_95percent, facecolor='yellow', alpha=1,
                     step='post')

    # add 68 % interval t the figure:
    plt.axvline(NCatmo_16, linestyle=':', label='16% probability limit = {0:.4f}'.format(NCatmo_16), color='b')
    plt.axvline(NCatmo_84, linestyle=':', label='84% probability limit = {0:.4f}'.format(NCatmo_84), color='b')
    ...
    plt.fill_between(bins8_small, 0, n_NCatmo_small, where=int_NCatmo_68percent, facecolor='green', alpha=1,
                     step='post')
    ...
    n_CCatmo_C12, bins9, patches9 = plt.hist(CCatmo_C12_mode, bins=Bins9, histtype='step', color='b',
                                             label='number of virt. experiments = {0:.0f}'
                                             .format(number_of_entries))
    plt.axvline(CCatmo_C12_expected, linestyle='dashed', label='expected number of events from simulation = {0:.3f}'
                .format(CCatmo_C12_expected), color='r')
    plt.axvline(CCatmo_C12_mean, linestyle='dashed', label='mean of the distribution = {0:.4f}'.format(CCatmo_C12_mean),
                color='b')
    # define interpolation function:
    f_CCatmo_C12 = interp1d(bins9[:-1], n_CCatmo_C12, kind='previous')
    bins9_small = np.arange(bins9[0], bins9[-2], 0.01)
    n_CCatmo_small = f_CCatmo_C12(bins9_small)

    # add 95 % interval to the figure:
    plt.axvline(CCatmo_C12_2_5, linestyle='-.', label='2.5% probability limit = {0:.4f}'.format(CCatmo_C12_2_5),
                color='b')
    plt.axvline(CCatmo_C12_97_5, linestyle='-.', label='97.5% probability limit = {0:.4f}'.format(CCatmo_C12_97_5),
                color='b')
    # define a boolean array, which is True between CCatmo_2_5 and CCatmo_97_5 (array of bool):
    int_CCatmo_95percent = (bins9_small >= CCatmo_C12_2_5) * (bins9_small <= CCatmo_C12_97_5)
    plt.fill_between(bins9_small, 0, n_CCatmo_small, where=int_CCatmo_95percent, facecolor='yellow', alpha=1,
                     step='post')

    # add 68 % interval t the figure:
    plt.axvline(CCatmo_C12_16, linestyle=':', label='16% probability limit = {0:.4f}'.format(CCatmo_C12_16),
                color='b')
    plt.axvline(CCatmo_C12_84, linestyle=':', label='84% probability limit = {0:.4f}'.format(CCatmo_C12_84),
                color='b')
    # define a boolean array, which is True between CCatmo_16 and CCatmo_84 (array of bool):
    int_CCatmo_68percent = (bins9_small >= CCatmo_C12_16) * (bins9_small <= CCatmo_C12_84)
    plt.fill_between(bins9_small, 0, n_CCatmo_small, where=int_CCatmo_68percent, facecolor='green', alpha=1,
                     step='post')

    plt.xticks(np.arange(15, np.max(CCatmo_C12_mode) + 2.5, 2.5))
    plt.xlabel("total number of observed atmo. CC background events")
    plt.ylabel("counts")
    plt.title("Distribution of the observed number of atmospheric CC background events on $^{12}$C" +
              " in virtual experiments "
              "\n(for DM mass of {2:.1f} MeV and in the energy window from {0:.1f} MeV to {1:.1f} MeV)"
              .format(lower_energy_bound, upper_energy_bound, DM_mass))
    plt.legend()
    plt.savefig(path_result + "/result_CCatmo_C12.png")
    plt.close()
