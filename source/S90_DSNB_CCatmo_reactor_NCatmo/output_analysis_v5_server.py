""" Script "output_analysis_v5_server.py" from 12.05.2020.
    ...
    done with analyze_spectra_v7_server2.py!

    Script uses the function output_analysis() from output_analysis_v5.py,
    but is optimized to run on the IHEP cluster.

    In analyze_spectra_v7_server2.py the dataset and the simulated spectrum were analyzed with
    Markov Chain Monte Carlo (MCMC) sampling and the results of the analysis are saved in the
    files DatasetX_mcmc_analysis.txt

    In analyze_spectra_v7_server2.py the mode of the total number of signal and background events are determined by
    ...
    - sys.argv[2] directory of the correct folder = "/junofs/users/dblum/work/signal_DSNB_CCatmo_reactor_NCatmo"
...
import numpy as np
import sys
from output_analysis_v5 import output_analysis
...
if SAVE_DATA:
    ...
    # Save the array FN_mode to txt file:
