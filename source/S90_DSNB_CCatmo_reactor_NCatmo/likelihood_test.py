""" test:
    ...
    chi^2 given by formula chi^2 = 2 * sum(n_i * ln(n_i/v_i) + v_i - n_i)

    v_i:    fixed background (DSNB + CCatmo + reactor + NCatmo): the number of different background events is fixed and
            not fitted as free parameter!

    n_i:    signal and background (S * f_s + v_i): number of signal events S is free parameter, f_s is spectral
            shape of signal and v_i fixed background

    Only 1 free parameter: number of signal events S.

    chi^2 is calculated for different values of S -> chi^2(S)

    value of S is determined for chi^2(S) = 0.9 -> S_90

    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ERROR:
    number of background events must be also fitted and therefore the numbers of background events must be free
    parameters!

    n_i must be fixed and v_i must contain free parameters, because the number of observed events n_i should be fix
    after a measurement!

    Do not use likelihood_test.py to determine 90 % upper limit of signal contribution.
