...
""" Script "analyze_spectra_v7_server2.py" from 12.05.2020.
    ...
    This script will be included to the auto_simu_analysis_S90_v3.sh script to
    automatically simulate datasets and analyze them for different DM masses.

    Script is based on the analyze_spectra_v7_wo_datasets.py Script, but changed a bit to be able to run it on the cluster.
    ...
    Dataset (virtual experiments) are generated with gen_dataset_v3_local.py
    Simulated spectra are generated with gen_spectrum_v4.py

    to run the script:

    python /junofs/users/dblum/work/S90_DSNB_CCatmo_reactor_NCatmo_FN/analyze_spectra_v7_server2.py 0 DM_mass

    give 3 arguments to the script:
    - sys.argv[0] name of the script = analyze_spectra_v7_server2.py
...
for number in np.arange(dataset_start, dataset_stop + 1, 1):
    ...
    # (np.array of float):
    parameter_guess = np.array([S_true, B_DSNB_true, B_CCatmo_p_true, B_CCatmo_C12_true, B_Reactor_true, B_NCatmo_true])
...
if SAVE_DATA:
    ...
    np.savetxt(path_analysis + '/info_mcmc_analysis_{0:d}_{1:d}.txt'.format(dataset_start, dataset_stop),
               np.array([DM_mass, min_E_cut, max_E_cut, interval_E_visible, S_true, S_max, B_DSNB_true,
                         B_CCatmo_p_true, B_CCatmo_C12_true, B_Reactor_true, B_NCatmo_true,
               ...
               header='General information about the MCMC analysis of virtual experiment to the expected spectra '
                      '((job_number = {10}, analyzed with analyze_spectra_v7_server2.py, {0}):\n'
               ...
               .format(now, dataset_start, dataset_stop, path_dataset, file_signal, file_DSNB, file_CCatmo_p,
                       file_reactor, file_NCatmo, file_CCatmo_C12, job_number))
