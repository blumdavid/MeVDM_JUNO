...
""" Script "analyze_spectra_v8_server2.py" from 13.08.2020.
    ...
    This script will be included to the auto_simu_analysis_S90_v4.sh script to
    automatically simulate datasets and analyze them for different DM masses.

    Script is based on the analyze_spectra_v8_wo_datasets.py Script, but changed a bit to be able to run it on the cluster.

    Difference between version 8 and version 7:
                reactor background is neglected, since there are 0 reactor events after PSD

    Difference between version 7 and version 6:
                atmospheric CC background on C12 is included, but fast neutron background not.
    ...
    Dataset (virtual experiments) are generated with gen_dataset_v4_local.py
    Simulated spectra are generated with gen_spectrum_v4.py

    to run the script:

    python /junofs/users/dblum/work/S90_DSNB_CCatmo_reactor_NCatmo_FN/analyze_spectra_v8_server2.py 0 DM_mass

    give 3 arguments to the script:
    - sys.argv[0] name of the script = analyze_spectra_v8_server2.py
...
file_signal = path_simu + "/signal_DMmass{0}_bin1000keV_PSD.txt".format(DM_mass)
Spectrum_signal = np.loadtxt(file_signal)
file_info_signal = path_simu + "/signal_info_DMmass{0}_bin1000keV_PSD.txt".format(DM_mass)
...
# atmo. CC background on protons in events/bin:
...
file_NCatmo = path_simu + "/NCatmo_onlyC12_wPSD99_bin1000keV_fit.txt"
...
# min_E_cut = DM_mass - 5
# max_E_cut = DM_mass + 5
# INFO-me: all entries of all spectra below 16 MeV are 0, which leads to an error. Therefore take energies above 16 MeV
min_E_cut = 16.0
...
# expected number of CCatmo background events on p in the energy window (float):
...
# Fraction of CCatmo background on p (np.array of float):
...
for number in np.arange(dataset_start, dataset_stop + 1, 1):
    ...
    # get the sample-chain of the atmo. CC background contribution on proton (np.array of float):
    ...
    if SAVE_DATA:
        ...
        np.savetxt(path_analysis + '/Dataset{0}_mcmc_analysis.txt'.format(number),
                   ...
                   header='Results of the MCMC analysis of virtual experiment (Dataset_{0:d}) to the expected spectrum'
                          '(job_number = {4}, analyzed with analyze_spectra_v7_server2.py, {1}):\n'
                          ...
                          'mode of the number of atmospheric CC background events on proton,\n'
...
if SAVE_DATA:
    ...
    np.savetxt(path_analysis + '/info_mcmc_analysis_{0:d}_{1:d}.txt'.format(dataset_start, dataset_stop),
               ...
               header='General information about the MCMC analysis of virtual experiment to the expected spectra '
                      '((job_number = {9}, analyzed with analyze_spectra_v8_server2.py, {0}):\n'
                      ...
                      'Prior Prob. of DSNB bkg: Gaussian with mean=B_DSNB_true and sigma = B_DSNB_true * 2\n'
                      'Corresponding to page 16 in the GERDA paper -> "very poorly known background"\n'
                      'Prior Prob. of atmo. CC bkg on p: '
                      'Gaussian with mean=B_CCatmo_true and sigma = B_CCatmo_true * 2\n'
                      ...
                      'sample):'
               .format(now, dataset_start, dataset_stop, path_dataset, file_signal, file_DSNB, file_CCatmo_p,
                       file_NCatmo, file_CCatmo_C12, job_number))
    ...
    np.savetxt(path_analysis + '/acceptance_fraction_sampling_{0:d}_{1:d}.txt'.format(dataset_start, dataset_stop),
               ...
               header='Mean values of the acceptance fraction during sampling of the sample from the MCMC analysis of '
                      ...
                      '(analyzed with analyze_spectra_v8_server2.py, {2})\n'
