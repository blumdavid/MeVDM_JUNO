""" Script "limit_sigma_annihilation_v4.py" from 14.08.2020.
    ...
    Results of the simulation and analysis saved in folder "S90_DSNB_CCatmo_NCatmo"
...
path_expected_spectrum_NC = path_expected_spectrum
...
for mass in DM_mass:
    ...
    """ Information about the work saved in dataset_output_{}: 
        ...
        - expected spectrum of the signal is saved in file: signal_DMmass{}_bin1000keV_PSD.txt
        - information about the signal spectrum is saved in file: signal_info_DMmass{}_bin1000keV_PSD.txt
    """
    # load information about the signal spectrum (np.array of float):
    info_signal = np.loadtxt(path_expected_spectrum + "/signal_info_DMmass{0}_bin1000keV_PSD.txt".format(mass))
    ...
    N_signal_wo_PSD = np.sum(np.loadtxt(path_expected_spectrum + "/signal_DMmass{0}_bin1000keV.txt".format(mass)))
    # number of signal events after PSD:
    N_signal_w_PSD = np.sum(np.loadtxt(path_expected_spectrum + "/signal_DMmass{0}_bin1000keV_PSD.txt".format(mass)))
...
np.savetxt(path_folder + "/limit_annihilation_JUNO.txt", limit_sigma_anni, fmt='%.5e',
           header="90 % upper limit of the DM annihilation cross-section from JUNO experiment in cm^3/2:\n"
                  "Values calculated with script limit_sigma_annihilation_v4.py;\n"
                  "Analysis done with script analyze_spectra_v8_server2.py and auto_output_analysis_v4.py;\n"
                  ...
                  "DSNB_bin1000keV_f027_PSD.txt,\n"
                  "CCatmo_onlyP_Osc1_bin1000keV_PSD.txt,\n"
                  "CCatmo_onlyC12_Osc1_bin1000keV_PSD.txt,\n"
                  "NCatmo_onlyC12_wPSD99_bin1000keV_fit.txt;\n"
                  "DM masses in MeV = {1};\n"
...
if JAVG:
    ...
    # plot canonical:
    plt.semilogy(DM_mass, limit_sigma_anni_Javg_canonical, marker='x', markersize='6.0', linestyle='-', color='red',
                 linewidth=2.0, label='canonical value $J_{avg}$' + '={0:.1f}'.format(J_avg_canonical))
    # plot maximum:
    plt.semilogy(DM_mass, limit_sigma_anni_Javg_MQGSL_max, marker='x', markersize='6.0', linestyle=':', color='red',
