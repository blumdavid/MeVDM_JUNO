""" Script "output_analysis_v6_server.py" from 13.08.2020.
    ...
    done with analyze_spectra_v8_server2.py!

    Script uses the function output_analysis() from output_analysis_v6.py,
    but is optimized to run on the IHEP cluster.

    In analyze_spectra_v8_server2.py the dataset and the simulated spectrum were analyzed with
    Markov Chain Monte Carlo (MCMC) sampling and the results of the analysis are saved in the
    files DatasetX_mcmc_analysis.txt

    In analyze_spectra_v8_server2.py the mode of the total number of signal and background events are determined by
    MCMC sampling of the posterior probability

    give 7 arguments to the script:
    - sys.argv[0] name of the script = output_analysis_v6_server.py
    - sys.argv[1] Dark matter mass in MeV
    - sys.argv[2] directory of the correct folder = "/junofs/users/dblum/work/signal_DSNB_CCatmo_reactor_NCatmo_FN"
...
import numpy as np
import sys
from output_analysis_v6 import output_analysis

""" Set boolean value to define, if the result of output_analysis_v4_server.py is saved: """
SAVE_DATA = True
...
file_info_analysis = path_analysis + "/info_mcmc_analysis_0_{0}.txt".format(number_of_datasets_per_job-1)
...
""" display and analyze the results from the analysis with function output_analysis() from output_analysis_v5.py """
(number_of_entries, lower_energy_bound, upper_energy_bound, S_mode, S_50, S_50_sigma, S_50_16, S_50_84, signal_expected,
 ...
 DSNB_expected, CCatmo_p_mode, CCatmo_p_50, CCatmo_p_50_sigma, CCatmo_p_50_16, CCatmo_p_50_84, CCatmo_p_expected,
 NCatmo_mode, NCatmo_50, NCatmo_50_sigma, NCatmo_50_16, NCatmo_50_84, NCatmo_expected,
 CCatmo_C12_mode, CCatmo_C12_50, CCatmo_C12_50_sigma, CCatmo_C12_50_16, CCatmo_C12_50_84, CCatmo_C12_expected,
...
if SAVE_DATA:
    ...
    # Save the array CCatmo_p_mode to txt file:
    np.savetxt(path_output + "/result_mcmc/CCatmo_p_mode_DMmass{0}.txt".format(DM_mass),
               CCatmo_p_mode, fmt="%4.5f",
               header="Observed number of CCatmo background events on protons (mode of B_CCatmo_p) "
                      "in virtual experiments\n"
                      "(in the energy window from {0:.1f} MeV to {1:.1f} MeV)"
               .format(lower_energy_bound, upper_energy_bound))

    # Save the array NCatmo_mode to txt file:
    np.savetxt(path_output + "/result_mcmc/NCatmo_mode_DMmass{0}.txt".format(DM_mass),
               NCatmo_mode, fmt="%4.5f",
               header="Observed number of NCatmo background events (mode of B_NCatmo) in virtual experiments\n"
                      "(in the energy window from {0:.1f} MeV to {1:.1f} MeV)"
               .format(lower_energy_bound, upper_energy_bound))

    # Save the array CCatmo_C12_mode to txt file:
    np.savetxt(path_output + "/result_mcmc/CCatmo_C12_mode_DMmass{0}.txt".format(DM_mass),
               CCatmo_C12_mode, fmt="%4.5f",
               header="Observed number of CCatmo background events on C12 (mode of B_CCatmo_C12) "
                      "in virtual experiments\n"
                      "(in the energy window from {0:.1f} MeV to {1:.1f} MeV)"
               .format(lower_energy_bound, upper_energy_bound))
