""" Script to calculate 90 % upper limit of spectrum with Bayesian analysis and MCMC
    ...
    Difference between version 8 and 7:
        -   reactor background is neglected, since 0 reactor events remain after PSD cut
...
# mass_DM = [15.0]
mass_DM = np.arange(20, 100+5, 5)

# output path:
path_analysis = "/home/astro/blum/PhD/work/MeVDM_JUNO/S90_DSNB_CCatmo_NCatmo/test_Bayesian_ohne_datasets/"
...
spectrum_DSNB_per_bin = Spectrum_DSNB[entry_min_E_cut: (entry_max_E_cut + 1)] * 0.1
spectrum_CCatmo_p_per_bin = Spectrum_CCatmo_p[entry_min_E_cut: (entry_max_E_cut + 1)] * 0.1
spectrum_CCatmo_C12_per_bin = Spectrum_CCatmo_C12[entry_min_E_cut: (entry_max_E_cut + 1)] * 0.1
spectrum_NCatmo_per_bin = Spectrum_NCatmo[entry_min_E_cut: (entry_max_E_cut + 1)] * 0.1
...
def ln_likelihood(param, data, fraction_signal, fraction_dsnb, fraction_ccatmo_p, fraction_ccatmo_c12, fraction_ncatmo):
    """
    ...
    s, b_dsnb, b_ccatmo_p, b_ccatmo_c12, b_ncatmo = param

    # calculate the variable lambda_i(s, b_dsnb, b_ccatmo_p, b_ccatmo_c12, b_ncatmo),
...
def ln_priorprob(param):
    ...
    if b_dsnb >= 0.0:
        ...
        # sigma_b_dsnb = B_DSNB_true * 0.1
        sigma_b_dsnb = B_DSNB_true * 0.4
    ...
    if b_ccatmo_p >= 0.0:
        ...
        # sigma_b_ccatmo_p = B_CCatmo_p_true * 0.1
        sigma_b_ccatmo_p = B_CCatmo_p_true * 0.25
    ...
    if b_ccatmo_c12 >= 0.0:
        ...
        # sigma_b_ccatmo_c12 = B_CCatmo_C12_true * 0.1
        sigma_b_ccatmo_c12 = B_CCatmo_C12_true * 0.25
    ...
    if b_ncatmo >= 0.0:
        ...
        # sigma_b_ncatmo = B_NCatmo_true * 0.1
        sigma_b_ncatmo = B_NCatmo_true * 0.29
    ...
    return ln_prior_s + ln_prior_b_dsnb + ln_prior_b_ccatmo_p + ln_prior_b_ccatmo_c12 + ln_prior_b_ncatmo
...
for mass in mass_DM:
    ...
    file_signal_woPSD = path_simu + "/signal_DMmass{0:.0f}_bin1000keV.txt".format(mass)
    Spectrum_signal_woPSD = np.loadtxt(file_signal_woPSD)
    spectrum_Signal_per_bin_woPSD = Spectrum_signal_woPSD[entry_min_E_cut: (entry_max_E_cut + 1)] * 0.1
    ...
    file_signal = path_simu + "/signal_DMmass{0:.0f}_bin1000keV_PSD.txt".format(mass)
    Spectrum_signal = np.loadtxt(file_signal)
    file_info_signal = path_simu + "/signal_info_DMmass{0:.0f}_bin1000keV_PSD.txt".format(mass)
    info_signal = np.loadtxt(file_info_signal)

    """ Get Dark Matter mass from the info_signal file: """
    DM_mass = info_signal[10]

    spectrum_Signal_per_bin = Spectrum_signal[entry_min_E_cut: (entry_max_E_cut + 1)] * 0.1

    # expected number of signal events in the energy window (float):
    S_true = np.sum(spectrum_Signal_per_bin)

    # Calculate PSD efficiency for this DM mass and append it to PSD_eff_array:
    ...
    Data = spectrum_DSNB_per_bin + spectrum_CCatmo_p_per_bin + spectrum_CCatmo_C12_per_bin + spectrum_NCatmo_per_bin

    # guess of the parameters (as guess, the total number of events from the simulated spectrum are used)
    # (np.array of float):
    parameter_guess = np.array([S_true, B_DSNB_true, B_CCatmo_p_true, B_CCatmo_C12_true, B_NCatmo_true])
    ...
    (S_maxlikeli, B_dsnb_maxlikeli, B_ccatmo_p_maxlikeli, B_ccatmo_c12_maxlikeli, B_ncatmo_maxlikeli) = result["x"]
    ...
    ax4.axhline(y=B_CCatmo_C12_true, color='b', linestyle='--')
    ax4.set_ylabel('$B_{CCatmo}$ on C12')
    ...
    fig1 = corner.corner(samples,
                         labels=["$S$", "$B_{DSNB}$", "$B_{CCatmo,p}$", "$B_{CCatmo,^{12}C}$", "$B_{NCatmo}$"],
                         labels_kwargs={"fontsize": 20},
                         truths=[S_true, B_DSNB_true, B_CCatmo_p_true, B_CCatmo_C12_true, B_NCatmo_true],
                         truth_color='b')
    # plt.show(fig1)
    # save figure:
    ...
    np.savetxt(path_analysis + '/DMmass{0:.0f}_mcmc_analysis.txt'.format(mass),
               np.array([S_mode, S_90, DSNB_mode, CCatmo_p_mode, CCatmo_c12_mode, NCatmo_mode,
...
np.savetxt(path_analysis + '/info_mcmc_analysis.txt',
           np.array([min_E_cut, max_E_cut, interval_E_visible, S_true, S_max, B_DSNB_true,
                     B_CCatmo_p_true, B_CCatmo_C12_true, B_NCatmo_true,
                     nwalkers, value_of_a, number_of_steps, step_burnin]),
           fmt='%4.5f',
           header='General information about the MCMC analysis to the expected spectra '
                  '(analyzed with analyze_spectra_v8_wo_dataset.py, {0}):\n'
                  ...
                  'Prior Prob. of DSNB bkg: Gaussian with mean=B_DSNB_true and sigma = B_DSNB_true * 0.1\n'
                  'Prior Prob. of atmo. CC bkg on p: '
                  'Gaussian with mean=B_CCatmo_true and sigma = B_CCatmo_true * 0.1\n'
                  'Prior Prob. of atmo. CC bkg on C12: '
                  'Gaussian with mean=B_CCatmo_true and sigma = B_CCatmo_true * 0.1\n'
                  'Prior Prob. of atmo. NC bkg: Gaussian with mean=B_NCatmo_true and sigma = B_NCatmo_true * 0.1\n'
                  'Values below:\n'
                  ...
                  'Expected number of CC atmospheric background events on protons in this energy range,\n'
                  'Expected number of CC atmospheric background events on C12 in this energy range,\n'
                  ...
                  'sample):'
           .format(now, file_signal, file_DSNB, file_CCatmo_p, file_NCatmo, file_CCatmo_C12))
...
for index in range(len(mass_DM)):
    ...
    limit_flux_90 = limit_neutrino_flux_v2(S_90_array[index], mass_DM[index], N_target, time_s, epsilon_IBD,
                                           epsilon_mu_veto, PSD_eff_array[index], MASS_NEUTRON, MASS_PROTON,
                                           MASS_POSITRON)
...
np.savetxt(path_analysis + "/limit_annihilation_JUNO_wo_datasets.txt", limit_anni_90_array, fmt='%.5e',
           ...
           .format(path_analysis, mass_DM, time_year, N_target, epsilon_IBD, epsilon_mu_veto, J_avg, file_DSNB,
                   file_CCatmo_p, file_CCatmo_C12, file_NCatmo))
