""" Script gen_dataset_v4.py (13.08.2020):
...
def compare_5fileinputs(signal, dsnb, ccatmo_p, ncatmo, ccatmo_c12, output_string):
    """
    ...
    if signal == dsnb == ccatmo_p == ncatmo == ccatmo_c12:
        output = signal
    else:
...
def gen_dataset_v4(mass_dm, save_data_txt, save_data_all, display_data, dataset_start, dataset_stop, path_output,
    ...
    """ Load the calculated spectrum of the atmospheric CC background on C12 from the txt-file: """
    # load calculated spectrum from corresponding file (events/bin) (np.array of float):
    ...
    interval_e_visible = compare_5fileinputs(interval_e_visible_signal, interval_e_visible_dsnb,
                                             interval_e_visible_ccatmo_p, interval_e_visible_ncatmo,
                                             interval_e_visible_ccatmo_c12, 'interval_e_visible')
    # check min_e_visible:
    min_e_visible = compare_5fileinputs(min_e_visible_signal, min_e_visible_dsnb, min_e_visible_ccatmo_p,
                                        min_e_visible_ncatmo, min_e_visible_ccatmo_c12, 'min_e_visible')
    # check max_e_visible:
    max_e_visible = compare_5fileinputs(max_e_visible_signal, max_e_visible_dsnb, max_e_visible_ccatmo_p,
                                        max_e_visible_ncatmo, max_e_visible_ccatmo_c12, 'max_e_visible')
    # check t_years (exposure time in years):
    t_years = compare_5fileinputs(t_years_signal, t_years_dsnb, t_years_ccatmo_p, t_years_ncatmo,
    ...
    spectrum_total_per_bin = (spectrum_signal_per_bin + spectrum_dsnb_per_bin +
                              spectrum_ccatmo_p_per_bin + spectrum_ncatmo_per_bin + spectrum_ccatmo_c12_per_bin)
    ...
    if save_data_txt:
        ...
        np.savetxt(path_dataset + '/info_dataset_{0:d}_to_{1:d}.txt'.format(dataset_start, dataset_stop),
                   ...
                   .format(dataset_start, dataset_stop, file_signal, file_dsnb, file_ccatmo_p, file_ncatmo,
                           file_ccatmo_c12))
    ...
    if save_data_all:
        ...
        np.savetxt(path_output + '/spectrum_simulated.txt', spectrum_total_per_bin, fmt='%4.5f',
                   header='Total simulated spectrum in events/bin:\n'
                          ...
                          '{0},\n{1},\n{2},\n{3},\n{4})'.format(file_signal, file_dsnb, file_ccatmo_p,
