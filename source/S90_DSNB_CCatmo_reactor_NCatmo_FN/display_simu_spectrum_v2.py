...
for dm_mass in masses:
    ...
    file_DSNB = path_spectra + "DSNB_bin500keV.txt"
    file_DSNB_info = path_spectra + "DSNB_info__bin500keV.txt"
    file_CCatmo = path_spectra + "CCatmo_Osc1_bin500keV.txt"
    file_CCatmo_info = path_spectra + "CCatmo_info_Osc1_bin500keV.txt"
    file_CCatmo_total = path_spectra + "CCatmo_total_Osc1_bin500keV.txt"
    ...
    file_DSNB_PSD = path_spectra + "DSNB_bin500keV_PSD.txt"
    file_DSNB_info_PSD = path_spectra + "DSNB_info__bin500keV_PSD.txt"
    ...
    file_CCatmo_total_PSD = path_spectra + "CCatmo_total_Osc1_bin500keV_PSD.txt"
    ...
    CCatmo_per_bin = np.loadtxt(file_CCatmo)
    CCatmo_total_per_bin = np.loadtxt(file_CCatmo_total)
    ...
    CCatmo_per_bin_PSD = np.loadtxt(file_CCatmo_PSD)
    CCatmo_total_per_bin_PSD = np.loadtxt(file_CCatmo_total_PSD)
    ...
    # calculate CCatmo specturm only on C12 (CCatmo_total - CCatmo_only_proton):
    CCatmo_C12_per_bin = CCatmo_total_per_bin - CCatmo_per_bin
    CCatmo_C12_per_bin_PSD = CCatmo_total_per_bin_PSD - CCatmo_per_bin_PSD
    ...
    spectrum_per_bin = (signal_per_bin + DSNB_per_bin + CCatmo_per_bin + CCatmo_C12_per_bin + reactor_per_bin +
                        NCatmo_per_bin + fast_neutron_per_bin)
    spectrum_per_bin_PSD = (signal_per_bin_PSD + DSNB_per_bin_PSD + CCatmo_per_bin_PSD + CCatmo_C12_per_bin_PSD +
                            reactor_per_bin_PSD + NCatmo_per_bin_PSD + fast_neutron_per_bin_PSD)
    ...
    plt.semilogy(E_visible, signal_per_bin, 'r-', label='signal from DM annihilation for '
                 ...
                 drawstyle="steps")
    plt.semilogy(E_visible, DSNB_per_bin, 'b-', label='DSNB (expected events = {0:.2f})'
                 .format(N_DSNB), drawstyle="steps")
    plt.semilogy(E_visible, reactor_per_bin, 'c-', label='reactor bkg (expected events = {0:.2f})'
                 .format(N_reactor), drawstyle="steps")
    plt.semilogy(E_visible, CCatmo_per_bin, 'g-',
                 label='atmo. CC bkg on p (expected events = {0:.2f})'
                 .format(N_CCatmo), drawstyle="steps")
    plt.semilogy(E_visible, CCatmo_C12_per_bin, 'g--',
                 label='atmo. CC bkg on C12 (expected events = {0:.2f})'
                 .format(N_CCatmo_C12), drawstyle="steps")
    plt.semilogy(E_visible, NCatmo_per_bin, color='orange', drawstyle="steps",
                 label='atmo. NC bkg (expected events = {0:.2f})'.format(N_NCatmo))
    plt.semilogy(E_visible, fast_neutron_per_bin, 'm-', label='fast neutron bkg (expected events = {0:.2f})'
    ...
    plt.semilogy(E_visible, DSNB_per_bin_PSD, 'b-', label='DSNB (expected events = {0:.2f})'
                 .format(N_DSNB_PSD), drawstyle="steps")
    plt.semilogy(E_visible, reactor_per_bin_PSD, 'c-', label='reactor bkg (expected events = {0:.2f})'
                 .format(N_reactor_PSD), drawstyle="steps")
    plt.semilogy(E_visible, CCatmo_per_bin_PSD, 'g-', label='atmo. CC bkg on p (expected events = {0:.2f})'
    ...
    plt.semilogy(E_visible, NCatmo_per_bin_PSD, color='orange', drawstyle="steps",
                 label='atmo. NC bkg (expected events = {0:.2f})'.format(N_NCatmo_PSD))
    plt.semilogy(E_visible, fast_neutron_per_bin_PSD, 'm-', label='fast neutron bkg (expected events = {0:.2f})'
    ...
    plt.xlabel("Visible energy in MeV")
    plt.ylabel("Expected spectrum dN/dE in events/bin (bin-width = {0:.2f} MeV)".format(E_vis_bin))
    plt.title("Expected spectrum in JUNO after {0:.0f} years of lifetime for {1:d} MeV Dark Matter signal with PSD\n"
              "(IBD supp. = {2:.2f} %, NC supp. = {3:.2f} %, FN supp. = {4:.2f} %)"
              .format(t_years, dm_mass, IBD_supp_total, NC_supp_total, FN_supp_total))
