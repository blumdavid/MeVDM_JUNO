""" Script "output_analysis_v4_server.py" from 30.10.2019.
    ...
    done with analyze_spectra_v6_server2.py!

    Script uses the function output_analysis() from output_analysis_v4.py,
    but is optimized to run on the IHEP cluster.

    In analyze_spectra_v6_server2.py the dataset and the simulated spectrum were analyzed with
    Markov Chain Monte Carlo (MCMC) sampling and the results of the analysis are saved in the
    files DatasetX_mcmc_analysis.txt

    In analyze_spectra_v6_server2.py the mode of the total number of signal and background events are determined by
    MCMC sampling of the posterior probability

    give 7 arguments to the script:
    - sys.argv[0] name of the script = output_analysis_v4_server.py
...
import numpy as np
import sys
from output_analysis_v4 import output_analysis
...
""" display and analyze the results from the analysis with function output_analysis() from output_analysis_v4.py """
(number_of_entries, lower_energy_bound, upper_energy_bound, S_mode, S_50, S_50_sigma, S_50_16, S_50_84, signal_expected,
 ...
 FN_mode, FN_50, FN_50_sigma, FN_50_16, FN_50_84, FN_expected,
...
if SAVE_DATA:
    ...
    np.savetxt(path_output + "/result_mcmc/FN_mode_DMmass{0}.txt".format(DM_mass),
               FN_mode, fmt="%4.5f",
               header="Observed number of FN background events (mode of B_FN) in virtual experiments\n"
                      "(in the energy window from {0:.1f} MeV to {1:.1f} MeV)"
               .format(lower_energy_bound, upper_energy_bound))
