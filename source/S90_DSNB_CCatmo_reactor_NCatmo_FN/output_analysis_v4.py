""" Script "output_analysis_v4.py" from 30.10.2019.
...
def output_analysis(save_data, dm_mass, path_output, path_analysis, file_info_analysis, file_number_start,
    ...
    # mode of the total number of fast neutron background events (np.array of float):
    fn_mode = np.array([])
    ...
    for number in np.arange(file_number_start, file_number_stop + 1, 1):
        ...
        value_mode_ncatmo = result_analysis[5]
        # get value of mode of B_FN (float):
        value_mode_fn = result_analysis[6]
        ...
        fn_mode = np.append(fn_mode, value_mode_fn)
    ...
    # calculate the mean and 16% and 84% confidence level of the array CCatmo_mode (float):
    ...
    # calculate the mean and 16% and 84% confidence level of the array FN_mode (float):
    fn_50 = np.mean(fn_mode)
    fn_50_sigma = np.std(fn_mode)
    fn_50_16, fn_50_84 = np.percentile(fn_mode, [16, 84])
    ...
    ncatmo_expected = information_analysis[9]
    # number of expected fast neutron background events in the energy window (float):
    fn_expected = information_analysis[10]
    ...
    if save_data:
        np.savetxt(path_output + "/result_mcmc/result_dataset_output_{0:d}.txt".format(dm_mass),
                   np.array([lower_energy_bound, upper_energy_bound, number_of_entries,
                             ...
                             fn_expected, fn_50, fn_50_sigma, fn_50_16, fn_50_84,
                   ...
                   header="Results of the analysis of the spectra in dataset_output_{3} "
                          "(with output_analysis_v4_server.py, {0}):\n"
